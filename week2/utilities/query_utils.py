...
def add_spelling_suggestions(query_obj, user_query):
    ...
    query_obj["suggest"] = {
        "text": user_query,
        "term_suggest": {
            "term": {"field": "suggest.text", "suggest_mode": "popular", "min_word_length": 3}
        },
        "phrase_suggest": {
            "phrase": {
    ...
    print(json.dumps(query_obj["suggest"], indent=4))
        ...
        if prior_clicks_for_query is not None and len(prior_clicks_for_query) > 0:
            ...
            if click_prior != "":
                click_prior_query_obj = (
                    None  # Implement a query object that matches on the ID or SKU with weights of
                )
